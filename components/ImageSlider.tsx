     ...
     <div className="h-96 flex flex-nowrap justify-start">
        ...
        {imageSliderData.map((data, index) => (
          <React.Fragment key={data.id}>
            ...
            <label htmlFor={data.id} className={`card ${selectedCard === data.id ? 'w-96' : 'w-20'} bg-cover cursor-pointer overflow-hidden flex items-end transition-all duration-600 ease-in-out shadow-lg m-2`} 
            
            onClick={() => handleCardClick(data.id)} style={{backgroundImage: `url('${data.image}')`}}>
